...
If there are multiple files open you can use `:qa` (Vim will prompt you to save or discard them)
...
The shortcut while in normal mode is `Shift ZQ`

To save changes and then quit, you can use either `:wq` or `:x`

or `Shift ZZ` is the shortcut (we'll be talking more about shortcuts)
...
Ex mode is different from reg/\ex mode, and substitution aka "search and replace"—and other such commands like `:g` or "global"—begin while in the Ex mode...
